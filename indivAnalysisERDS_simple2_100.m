function [thr, PSE,  lapse,thr_SE, bias_SE,menu,sec_est_thr,sec_est_thr_SE,dates,profileFound,...
            dataFile,fitQuality,r,prec,totalTime,x1,y1,yg1,pp,minChiInd,modelLabel]=indivAnalysisERDS_simple2_100(file, optResults,plotOrNot,nbFits, folder)
...
%fit the data by a psychometric curve to get an approximate of the threshold
       ...
       x1=sort(logic('union', Disparity_Targ_Bg,[])); %the disparity of the data to fit (truc pour enlever les doublons)
       x1 = x1(x1(:,1) >= -140 & x1(:,1) <= 140,:);

        for j=1:numel(x1)
             count(j)=sum(Disparity_Targ_Bg==x1(j));
             y1(j)=nanmean(Resp_Targ_Near(Disparity_Targ_Bg==x1(j))); %Disparity_Targ_Bg et Resp_Targ_Near sont ordonné de la même manière
             stdY(j)=sqrt(y1(j)*(1-y1(j)));
        end
        y1=y1';    %the data to fit
        count = count'; %the nb of data by dot
       % nn=mean(count); if nn~=count(1); warning('Be careful, we assume the same number of data for each point and some have a different count'); end 
