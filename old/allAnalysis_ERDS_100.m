...
function thres=allAnalysis_ERDS_100
    ...
    for i=listP % debut boucle sujet - liste des fichiers loades
        ...
        for f=1:numel(list)% du premier fichier au dernier de "list"
          a=a+1;
           ...
           [thr, PSE, ~,~, ~ ,~,~,~,~,~,... 
            ~,~,r,~,~,x1,y1,yg1,pp,minChiInd, modelLabel]=indivAnalysisERDS_simple2_100(fileI,[],0,nbFits); %reset 2 to 100 (nbrFits for analysis)
